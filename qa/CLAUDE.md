...
- User-reported bugs land with a reproducing scenario:
  ...
  - `bus_class_limit.py` — drives `struma-02-south` (car 140, bus 100) as
    ...
    `speed_limits.bus` is BG TOLL's whole `BE,C1,C1E,D,D1,D1E,DE` class (bus,
    minibus, truck 3.5-7.5 t, anything towing a trailer), which is why the
    Settings row is labelled for the class. Sibling of
    `vehicle_type_limit_badge.py` (truck, 90) and the **only** coverage of the
    100 km/h tier — so the spelled-number path through the hundreds ("one
    hundred" / "сто") is exercised here and nowhere else. Uses `COMBO_S5`, which
