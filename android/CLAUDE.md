...
**Unusable-zone guard** (`data/ZoneSanitizer.kt`, Swift twin `ios/.../SrednaBGData/ZoneSanitizer.swift`): every ingestion point in `ZoneRepository` — server sync, first-launch bundle load, bundle re-seed — filters the parsed catalog before it reaches Room, dropping zones with placeholder `(0, 0)` endpoints, fewer than 2 centerline points, a non-positive `distance_m`, or no car limit, and substituting the car limit for a missing truck/bus one (Gson zero-fills those, and a 0 km/h limit reads as permanently over-limit for that driver). **One bad zone must cost one zone, never all of them:** MapLibre rejects an entire `FeatureCollection` when a single feature is a zero-point LineString ("A line string must have two or more coordinate points"), so the two malformed Път I-8 zones the backend published in 2026-08 blanked all 76 zones on the map while sync happily reported success. `zonesFeatureCollection` in `ZoneMapScreen.kt` skips degenerate geometry as a second line of defence. Drops **and repairs** are logged separately on the QA channel (`SrednaBG.Loc`: `zones dropped (n=…) ids=[…] origin=…` / `zones repaired (n=…) …`, bodies identical on iOS), and `qa/scenarios/sync/zones_all_usable.py` fails the sync suite on either. The repair line is not cosmetic: a zone missing its truck/bus limit is invisible here and **fatal on the 1.x clients the stores serve** (iOS 1.x fails the entire `/api/zones` decode), so without reporting it, QA on a current build passes while every published install is broken. The real fix always belongs upstream in the scraper's publish guard. Regressions: `ZoneSanitizerTest`, `ZoneSanitizerTests.swift`.
...
**Data feed** — `zoneFeedVersion` at the top of `app/build.gradle.kts` (currently **1**) is the compile-time choice of which zone-data feed this build talks to. It drives two things from one literal: `BuildConfig.ZONE_FEED_VERSION`, which `ZoneApi.endpoint()` turns into `/api/zones` (feed 1, unsuffixed — never move it) or `/api/zones.N`; and which `backend/data/zones*.json` `prepareZonesAsset` bundles, so the offline catalog can't disagree with what the app syncs. `VersionResponse.unsupported` (any non-zero value) means that feed has been retired: `ZoneRepository.syncFromServer` persists it to `zone_feed_unsupported` on **every** successful version fetch — including `false`, so a re-supported feed clears its own notice — and `SettingsScreen` renders the "no longer receives updated zone data" line (testTag `settings-zone-data-unsupported`) under the hash row. QA flips it via the `zone_feed_unsupported` debug key (`qa/scenarios/ui/zone_feed_unsupported.py`); no live feed sets it. Swift twins: `BackendURLs.feedVersion`, `SettingsKey.zoneFeedUnsupported`. See `scrapers/CLAUDE.md` "Data feeds".
...
- Data: `ZoneRepository.kt` (+`SyncResult`), `ZoneSanitizer.kt` (+test), `ZoneDatabase.kt`, `ZoneSyncWorker.kt`, `ZoneSyncScheduler.kt`, `MapRepository.kt`, `MapSyncWorker.kt`; remote: `ZoneApi.kt` (`map_hash` in `VersionResponse`), `MapApi.kt` (`/api/map/bundle.zip`)
...
- **Play Store listing**: ✅ done (store description, content rating, screenshots uploaded). **1.1.0 (versionCode 10100) is live on Play**; the repo is at versionCode 20000 / 2.0.0 in preparation, so the next upload is a new AAB above it. Which versions are published on which channel is recorded once, in **`VERSIONS.md`** — don't restate it here. Play subtitle: Секционен контрол — средна скорост. Keywords: средна скорост, секционен контрол, камери, отсечки, БГ ТОЛ, скорост, автомагистрала. Play Store targets BG only. Screenshots are produced by the `/screenshot-app` + `/frame-screenshots` skills (raw PNGs from the emulator, then Waze-style framed PNGs from those — see `qa/CLAUDE.md` "Store-screenshot tooling" for the workflow).
