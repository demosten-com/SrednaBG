...
- **Vehicle types** — a `VehicleType` enum (car/truck/bus/motorcycle) in the core picks the per-vehicle speed limit per zone; motorcycle falls back to the car limit where a zone has none. These are **licence categories, not vehicle shapes**: BG TOLL publishes exactly three limits per zone, and `bus` is its `BE,C1,C1E,D,D1,D1E,DE` class — bus, minibus, truck 3.5–7.5 t, **and anything towing a trailer**. So the Settings picker has three rows labelled for the whole class (`Кола/Мотоциклет`, `Автобус/Бус/С ремарке`, `Камион`); a car with a caravan picks the middle one. There is deliberately no separate car-with-trailer type — it would be a second enum value resolving an identical limit. Each case carries a `setting` token (Kotlin) / raw value (Swift) that both platforms persist identically — do not derive it from the enum name. See `android/core/CLAUDE.md`
- **Trip history** — completed average-speed-zone traversals are recorded to an on-device DB (Android Room, iOS SwiftData) and browsed in a **History tab** (list → per-record detail with a speed-over-time graph). The graph draws the instantaneous speed, the running-average band, and the dashed zone limit, framed by a km/h Y axis + `m:ss` X axis. The stats (sustained min/max with spike suppression, running average, downsample) live in the shared pure core (`HistoryStats` in Kotlin, hand-ported to Swift) so both platforms compute identical numbers. Records are pruned by a `history_retention` setting (none / 1 / 3 / 6 months, default 3). The detail page has a **"Show on map"** top-bar action: it jumps to the Map tab, colors the traversed zone with the trip's verdict (green within / red over limit) and puts the location arrow at the zone start pointing toward the end. Disabled while tracking is active (the two must not drive the map at once) and when the record's zone no longer exists in the catalog; starting tracking clears the highlight on both platforms. See `android/CLAUDE.md`, `android/core/CLAUDE.md`, `ios/CLAUDE.md`
...
- **Data feeds** — the scraper can serve more than one payload variant at once: feed 1 is `/api/zones` + `/api/version` (what every published install fetches, and what `zones.json` is), feed N>1 is `/api/zones.N` + `zones.N.json`. A build picks its feed at compile time and never negotiates, so a shape or zone set the 1.x fleet can't parse becomes publishable the moment it lives on a feed those installs never request — the only escape from a wire format that is otherwise frozen forever (`scrapers/contracts/`). Retiring a feed stamps `"unsupported": 1` on its version file, which both apps surface as an "update the app" notice under the zone-data hash in Settings. Feeds are declared in `contracts/manifest.json` and mapped per release in `VERSIONS.md`'s Feed column. See `scrapers/CLAUDE.md` "Data feeds"
- **No certificate pinning** — sync calls (Android OkHttp, iOS `URLSession`) ride the platform TLS defaults, deliberately *not* pinned. Zone data is public and non-sensitive, so pinning buys little while adding cert-rotation fragility and F-Droid reproducibility friction. Platform defaults (HTTPS / ATS, cleartext denied on API 28+) suffice. Don't add pinning; if a reviewer flags it, point here.
