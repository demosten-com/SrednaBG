...
- **UI**: `ios/Packages/SrednaBGUI/Sources/SrednaBGUI/{Theme,L10n,Bundle+AppVersion,Components/{SpeedDisplay,StatusChip},Screens/{HomeScreen,SettingsScreen,AboutScreen,ZoneMapScreen,RootView,KeepScreenAwakeModifier},Map/MapLibreView}.swift` (the local `Theme.swift` is a one-line re-export of `SrednaBGTheme`; `Bundle+AppVersion` exposes `Bundle.appShortVersion` so the Settings + About screens render the live `CFBundleShortVersionString` instead of a hardcoded literal — matches Android's `BuildConfig.VERSION_NAME` usage).
...
- **Version bump**: `MARKETING_VERSION` is `1.0.3` across all targets (main app + widgets + tests + UI tests), matching the Android version. `CURRENT_PROJECT_VERSION` is `1` and increments per upload to App Store Connect.
