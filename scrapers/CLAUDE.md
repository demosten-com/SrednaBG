...
**Data feeds** (`src/feeds.py`). A *feed* is a served payload variant named by a positive integer: feed 1 is `/api/zones` + `/api/version`, feed N>1 is `/api/zones.N` + `/api/version.N` (files `zones.N.json` / `version.N.json`, each with its own snapshot history). Clients choose their feed at **compile time** (`BuildConfig.ZONE_FEED_VERSION` on Android, `BackendURLs.feedVersion` on iOS — the same literal also picks which `backend/data/zones*.json` the build bundles) and never negotiate, which is the entire point: a feed is only ever fetched by builds made for it, so a shape or a zone set the 1.x fleet cannot consume becomes publishable the moment it lives on a feed those installs don't request. Without this, the wire is frozen for as long as any 1.x install exists — indefinitely.

`contracts/manifest.json` declares the feeds and gives every client a `feed`; `publish_guard_errors` runs per feed against that feed's projected payload and enforces only the clients on it. Publishing is **all-or-nothing across feeds** — a half-updated `/api` (feed 1 fresh, feed 2 stale) is worse than a uniformly stale one, because nothing downstream can tell them apart. `feeds.PROJECTIONS` maps feed → a transform of the canonical `ZoneDatabase`; **feed 1's is the identity and must stay so**: `tests/test_feeds.py` pins its bytes and its hash against the committed `zones.json`, because a feed-1 hash that shifts merely because feeds exist would push the entire fleet through a pointless full resync. Each feed's hash covers *its own* projected zones, or a feed whose content differs would never signal a change to its clients. Retiring a feed means setting its manifest `status` to `unsupported`: it keeps being served, its `version*.json` gains `"unsupported": 1`, and both apps then show a "no longer receiving updated zone data" notice under the hash row in Settings (`status: retired` stops writing it altogether). Note that notice only reaches builds that shipped with the reader — 1.0.4 / 1.1.0 ignore the key, so retiring feed 1 will be silent for them. Only add a feed alongside its `VERSIONS.md` Feed column entry; see `contracts/README.md` "Feeds".
...
| `~/srednabg_com/api/zones.json` | Live, atomically replaced. Served as `/api/zones` (feed 1). |
| `~/srednabg_com/api/version.json` | Live, atomically replaced. Served as `/api/version` (feed 1). `map_hash` **must stay `null`** — see below. |
...
| `~/srednabg_com/api/{zones,version}.N.json` + `zones.N-<ts>.json` | The same three files for **feed N>1**, served at `/api/zones.N` / `/api/version.N`. Only written while that feed is declared in `contracts/manifest.json`; none exist today. |
...
`/api/version` carries `hash` (zones), `map_hash` (bundle), `zone_count`, and `feed` (the data-feed number this file serves); each hash gates its own re-fetch on the client. A retired feed additionally carries `"unsupported": 1` — see "Data feeds" above.
